...
def dashboard(request):
    ...
    context = {
        ...
        'dynamic_form_active': dynamic_form_active,
    }
    ...
    if role == 'faculty':
        ...
        dynamic_submissions = DynamicFormSubmission.objects.filter(
            faculty=request.user
...
def ccr_form(request):
    # Check if dynamic form is active (priority) or CCR form is active
    dynamic_form_active = check_dynamic_form_status()
    
    if dynamic_form_active:
        # Redirect to dynamic form
...
def dynamic_form(request):
    """
    Dynamic form view for faculty
    """
    # Check if dynamic form is active
    try:
        dynamic_form_obj = DynamicForm.objects.filter(status=DynamicForm.STATUS_ACTIVE).first()
        
        if not dynamic_form_obj:
            messages.error(request, "No active dynamic form available. Please contact administrator.")
            return redirect('dashboard')
            
    except Exception as e:
        messages.error(request, "Error accessing dynamic form. Please try again later.")
        return redirect('dashboard')
    ...
    # Check if faculty has assigned courses
    if not request.user.assigned_courses.exists():
        messages.error(request, "You don't have any assigned courses to submit forms for.")
        return redirect('dashboard')
    
    # Get questions for the active form
    questions = FormQuestion.objects.filter(form=dynamic_form_obj).order_by('order')
    
    if request.method == 'POST':
        # Handle dynamic form submission via AJAX in template
        pass
    
    assigned_courses = request.user.assigned_courses.all()
    
    return render(request, 'accounts/dynamic_form.html', {
        'dynamic_form': dynamic_form_obj,
        'questions': questions,
        'assigned_courses': assigned_courses,
        'form_active': True
    })
...
def ccr_submissions(request):
    ...
    dynamic_submissions = DynamicFormSubmission.objects.filter(faculty=request.user).select_related('course', 'dynamic_form').order_by('-submission_date')
